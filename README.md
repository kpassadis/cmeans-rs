...
Gemini said
Ah, the dreaded Dark Mode trap! I completely understand your frustration. What happens is that CodeCogs generates SVGs with transparent backgrounds and black text by default. On GitHub or crates.io in light mode, it looks great. But if your system or browser is in dark mode, you end up staring at black math equations on a dark gray background.

And you are totally right about the inline variables—since crates.io doesn't parse LaTeX, wrapping \alpha \in (1, \infty) in code backticks just renders the raw code string, which looks very messy next to the math blocks.

We can solve both of these natively:

The Math Blocks: We will add \bg_white to the CodeCogs URLs. This forces a crisp white background behind the equations so they pop beautifully no matter what theme the user has active.

The Inline Symbols: We will replace the raw LaTeX strings with proper Unicode characters and HTML <sub>/<sup> tags (which crates.io's Markdown parser does fully support!).

Here is the final, completely bulletproof README.md.

Step 1: Update the README
Replace your README contents with this:
